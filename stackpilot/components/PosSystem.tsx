...
const PosSystem: React.FC<PosSystemProps> = ({ cart, onUpdateQuantity, onRemoveFromCart, onClearCart, onCreateOrder }) => {
    ...
    if (discountType === 'percentage') {
        ...
        discountAmount = Math.min(subtotal, discount); // Cap discount at subtotal
    }
    ...
    const handleCreateOrder = (paymentMethod: 'cash' | 'card') => {
        onCreateOrder({
            items: cart,
            total,
            subtotal,
    ...
    return (
        <aside className="w-96 bg-gray-800/50 flex flex-col p-4 border-l border-gray-700">
            <header className="flex justify-between items-center mb-4">
                ...
                <button
                    onClick={() => { onClearCart(); setDiscount(0); }}
            ...
            <div className="flex-1 overflow-y-auto -mr-2 pr-2">
                {cart.length === 0 ? (
                    ...
                    <ul className="space-y-3">
                        {cart.map(item => (
                            <li key={item.id} className="flex items-center space-x-3 bg-gray-700/50 p-3 rounded-lg">
                               <div className="flex-1">
                                ...
                                <div className="flex items-center space-x-2">
                                    ...
                                    />
                                    <button onClick={() => onRemoveFromCart(item.productId)} className="p-1.5 text-red-500 hover:text-red-400 rounded-full hover:bg-red-500/10">
                 ...
                 <div className="flex justify-between items-center text-sm text-gray-300">
                    ...
                    <div className="flex items-center space-x-1">
                        <div className="flex items-center bg-gray-700 border border-gray-600 rounded-lg p-0.5">
                            <button onClick={() => setDiscountType('percentage')} className={`px-2 py-0.5 text-xs rounded-md ${discountType === 'percentage' ? 'bg-sky-500 text-white' : 'text-gray-400'}`}>%</button>
                            <button onClick={() => setDiscountType('fixed')} className={`px-2 py-0.5 text-xs rounded-md ${discountType === 'fixed' ? 'bg-sky-500 text-white' : 'text-gray-400'}`}>₱</button>
                        </div>
                        <input 
                            type="number" 
                            id="discount"
